...
function App() {
  ...
  const { activeProject, initializeProject, updateProject, loadProjects } = useProjectStore();
  ...
  const handleConcurrencyChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (activeProject) {
      try {
        const threads = Math.min(Math.max(parseInt(e.target.value), 1), 10);
        await updateProject(activeProject.id, { threads });
      } catch (err) {
